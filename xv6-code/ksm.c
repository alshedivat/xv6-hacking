...
// Update ksm_freebitmap setting bits to 1 (freeing) or to 0 (occupying)
...
// Set time funtion. Just for the purpose not to repeat the code.
...
static void destroy_seg(int hd) {
  if (!holding(&ksmlock))
    panic("destroyseg not holding ksmlock");
